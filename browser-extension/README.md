...
1. 解壓縮下載的 `Lumen-Trace-Opera-GX-0.4.0.zip`。
2. 在 Opera GX 網址列輸入 `opera:extensions`，按 Enter。
3. 開啟頁面右上方的「開發人員模式」。
4. 選擇「載入未封裝的擴充功能」（Load unpacked）。
5. 指定解壓後、裡面直接看得到 `manifest.json` 的 `Lumen-Trace-Opera-GX-0.4.0` 資料夾。
...
2. 按「原畫質下載並交接」。Opera 確認下載完成後會自動開啟 ChatGPT。
3. 在 ChatGPT 用迴紋針上傳最新的 `Lumen-Trace-IG-Story-*` 圖片；看到縮圖後再開一次 Lumen Trace。
...
- `downloads`：在 IG 限動的原來源檔交接，或圖片剪貼簿失敗的 PNG 備援時使用；收到 Opera 的完成狀態後才開啟 ChatGPT。

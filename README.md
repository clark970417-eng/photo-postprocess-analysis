...
The optional `browser-extension` folder adds a user-triggered shortcut for X and Instagram. Version 0.3.1 deliberately places only the cropped PNG on the clipboard, because a combined `image/png` + `text/plain` item can cause ChatGPT to choose the text representation and discard the photo. After the user pastes and explicitly confirms the photo thumbnail, reopening Lumen Trace on the ChatGPT tab copies the evidence-based Traditional Chinese analysis prompt as a separate step. A uniquely identified, single-active in-memory handoff is accepted for 30 minutes and never stores the source image URL or pixels; the short pre-capture reservation expires after two minutes. The extension attempts to delete the record after the prompt is copied. If image clipboard writing fails, a background service records its generated filename and download ID, can recover after a Manifest V3 worker restart, and opens ChatGPT only after Opera reports that the fallback download completed.

In Opera GX, open `opera:extensions`, enable Developer Mode, choose **Load unpacked**, and select the extracted `browser-extension` directory. The extension requests only `activeTab`, `clipboardWrite`, `contextMenus`, `downloads`, `scripting`, and `storage`; it does not request persistent `chatgpt.com` host access and does not inspect the ChatGPT page or conversation DOM. `downloads` is used only after PNG clipboard writing fails, and the background service waits for the local fallback download to complete before advancing. `storage.session` holds the handoff ID, prompt, platform label, dimensions, source-tab ID, image-transfer mode, status, and timestamp; download fallback also stores its generated filename and download ID. It never stores the image URL or pixels. The optional website action explicitly places the current page's source-image URL—which can contain access or signature parameters—in the Lumen Trace page query string, so it is separate from the private clipboard handoff.

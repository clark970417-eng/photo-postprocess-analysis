...
## Chrome, Opera GX, and Safari extension

The optional `browser-extension` folder is a Manifest V3 extension for Chrome, Opera GX, and Safari 16.4 or later. It adds a user-triggered shortcut for X and Instagram. In Chrome and Opera GX, version 0.5.0 can hand an Instagram Story's currently served image file directly to browser downloads without screenshotting, resizing, or re-encoding it; Instagram itself may already have compressed the upload. Safari doesn't expose the WebExtension downloads API, so its Story flow uses the visible-photo PNG handoff instead. Other pages deliberately place only the cropped PNG on the clipboard, because a combined `image/png` + `text/plain` item can cause ChatGPT to choose the text representation and discard the photo. After the user uploads or pastes and explicitly confirms the photo thumbnail, reopening Lumen Trace on the ChatGPT tab copies the evidence-based Traditional Chinese analysis prompt as a separate step.

For Chrome, open `chrome://extensions`; for Opera GX, open `opera:extensions`. Enable Developer Mode, choose **Load unpacked**, and select `browser-extension`. Current macOS Safari can temporarily install the same folder from Safari Settings → Developer → **Add Temporary Extension…**; Safari removes temporary extensions after 24 hours or when Safari quits. Production Safari distribution requires Apple's Safari Web Extension packager and an Xcode/App Store project. Full installation and platform-difference instructions are in [`browser-extension/README.md`](browser-extension/README.md).

The extension requests only `activeTab`, `clipboardWrite`, `contextMenus`, `downloads`, `scripting`, and `storage`; unsupported permissions are ignored by Safari. It does not request persistent `chatgpt.com` host access and does not inspect the ChatGPT page or conversation DOM. `storage.session` holds the handoff ID, prompt, platform label, dimensions, source-tab ID, image-transfer mode, status, timestamp, generated filename, and download ID. It never stores the image URL or pixels. The optional website action explicitly places the current page's source-image URL—which can contain access or signature parameters—in the Lumen Trace page query string, so it is separate from the private handoff.
...
├── browser-extension/       # Chrome, Opera GX, and Safari 16.4+ WebExtension
